...
#SBATCH --array=21-22                # 100 jobs, indexed 0 through 99
...
unalias conda
source /gpfsm/dnb34/pcastell/miniforge3/etc/profile.d/conda.sh
...
python generate_lut.py config.yaml --channel-start $START_CHANNEL --channel-stop $STOP_CHANNEL > logs/python_task_${SLURM_ARRAY_TASK_ID}.log 2>&1
